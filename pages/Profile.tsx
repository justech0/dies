...
export const Profile = () => {
    ...
    useEffect(() => {
        if (user) {
            ...
            // Since user object might not have all advisor fields directly in simple context, 
            // we should rely on what we have or fetch extended profile
            setFormData({
                name: user.name || '',
                email: user.email || '',
                phone: user.phone || '',
    ...
    const handleDeleteSelected = async () => {
        ...
        if (window.confirm(`${selectedListingIds.length} adet ilanı silmek istediğinize emin misiniz?`)) {
            try {
                ...
                // Keep selection mode open as per typical UX, or close it. 
            } catch (e) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const updatedData = {
                ...
                // We keep stats as they are read-only usually
            };
    ...
    // Active listings calculation for stats
                ...
                <div className="lg:col-span-4">
                    <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm sticky top-32">
                         <div className="flex flex-col items-center mb-8">
                            ...
                            <p className="text-xs text-gray-400">Değiştirmek için fotoğrafa tıklayın</p>
                        </div>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-5">
                            ...
                            <div>
                                ...
                                <input value={formData.specializations} onChange={e => setFormData({...formData, specializations: e.target.value})} className={inputClass} placeholder="Örn: Ticari Gayrimenkul, Arsa, Lüks Konut" />
                                <p className="text-[10px] text-gray-400 mt-1">Virgül ile ayırarak yazınız.</p>
                            </div>

                            <div>
                                <label className={labelClass}>Hakkımda (Biyografi)</label>
                                <textarea rows={4} value={formData.about} onChange={e => setFormData({...formData, about: e.target.value})} className={inputClass} placeholder="Kendinizden bahsedin..." />
                            </div>

                            <div><label className={labelClass}>Instagram Linki</label><input value={formData.instagram} onChange={e => setFormData({...formData, instagram: e.target.value})} className={inputClass} placeholder="https://instagram.com/..." /></div>
                            <div><label className={labelClass}>Facebook Linki</label><input value={formData.facebook} onChange={e => setFormData({...formData, facebook: e.target.value})} className={inputClass} placeholder="https://facebook.com/..." /></div>

                            <button disabled={isSavingProfile} className="w-full bg-dies-blue hover:bg-blue-900 text-white py-3.5 rounded-xl font-bold flex items-center justify-center gap-2 mt-4 transition-colors shadow-lg shadow-blue-900/10">
                                {isSavingProfile ? <Loader className="animate-spin" size={18} /> : <Save size={18} />} Değişiklikleri Kaydet
                            </button>
                        </form>
                    </div>
                </div>
                ...
                <div className="lg:col-span-8 space-y-6">
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-5 rounded-2xl border border-gray-100 shadow-sm">
                         <div>
                             ...
                             <p className="text-xs text-gray-400 mt-0.5">İlanlarınızı buradan yönetebilir ve düzenleyebilirsiniz.</p>
                         </div>
                         <div className="flex gap-3 w-full sm:w-auto">
                             <button onClick={() => { setIsSelectionMode(!isSelectionMode); setSelectedListingIds([]); }} className={`px-5 py-2.5 rounded-xl font-bold text-sm border transition-all flex items-center justify-center gap-2 ${isSelectionMode ? 'bg-gray-100 border-gray-200 text-gray-600' : 'bg-white border-gray-200 text-gray-600 hover:border-gray-300'}`}>
                                 {isSelectionMode ? 'Vazgeç' : 'Seç / İşlem Yap'}
                             </button>
                             <button onClick={() => navigate('/ilan-ver')} className="bg-dies-red text-white px-5 py-2.5 rounded-xl font-bold text-sm hover:bg-red-700 transition-all shadow-lg shadow-red-500/20 flex items-center justify-center gap-2">
                                 <Edit size={16} /> Yeni İlan
                             </button>
                         </div>
                    </div>

                    {/* Bulk Actions Bar - Matches screenshot style more closely */}
                    {isSelectionMode && (
                        <div className="bg-blue-50 border border-blue-100 p-4 rounded-xl flex flex-col md:flex-row justify-between items-center gap-4 animate-in fade-in slide-in-from-top-2">
                            ...
                            <div className="flex items-center gap-3 w-full md:w-auto">
                                <button disabled={selectedListingIds.length === 0} onClick={handleMarkAsSold} className="flex-1 md:flex-none flex items-center justify-center gap-2 px-5 py-2 bg-[#25D366] hover:bg-[#20b85a] text-white rounded-lg font-bold text-sm disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-sm">
                                    <CheckCircle size={16} /> Satıldı Olarak İşaretle
                                </button>
                                <button disabled={selectedListingIds.length === 0} onClick={handleDeleteSelected} className="flex-1 md:flex-none flex items-center justify-center gap-2 px-5 py-2 bg-dies-red hover:bg-red-700 text-white rounded-lg font-bold text-sm disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-sm">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {listings.map(listing => (
                            <div key={listing.id} className={`relative group transition-all duration-300 bg-white rounded-xl ${isSelectionMode && selectedListingIds.includes(listing.id) ? 'ring-2 ring-dies-blue transform scale-[0.99]' : ''}`}>
                                ...
                                {/* Specific Edit Button for Card - Only visible when NOT in selection mode */}
                                {!isSelectionMode && (
                                    <button 
