...
import { MapPin, Phone, Clock, Users, Building, ArrowRight, Inbox } from 'lucide-react';
// @ts-ignore
...
export const Offices = () => {
  ...
  useEffect(() => {
      ...
      api.offices.getList()
        .then(res => {
          if (res && Array.isArray(res)) setOffices(res);
        })
        .catch(console.error)
      ...
      <div className="container mx-auto px-4">
        ...
        {isLoading ? (
           <div className="space-y-12">
             {[1, 2].map(i => (
               <div key={i} className="max-w-6xl mx-auto h-[400px] bg-gray-200 rounded-3xl animate-pulse"></div>
             ))}
           </div>
        ) : offices.length > 0 ? (
                  ...
                  <div key={office.id} className="max-w-6xl mx-auto bg-white rounded-3xl shadow-xl overflow-hidden border border-gray-100">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 h-[250px] md:h-[500px]">
                          <div className="md:col-span-2 relative h-full group overflow-hidden">
                              <img src={office.image || 'https://via.placeholder.com/1200x800'} alt={office.name} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
          ...
          <div className="flex flex-col items-center justify-center py-20 bg-white rounded-3xl border border-dashed border-gray-200 text-center">
              ...
              <h3 className="text-xl font-bold text-dies-dark mb-1">Kayıtlı ofis bulunamadı</h3>
              <p className="text-gray-400">Yakında yeni lokasyonlarımızla hizmetinizde olacağız.</p>
          </div>
        )}
      </div>
    </MotionDiv>
